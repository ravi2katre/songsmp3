...
// for read directory -- rk
include("includes/admin-config.php");

if(!isset($_REQUEST['parent_cat_id'])):
/*RK - 20120930
*Form to moselect category for moving file
*/

/*Get category list */
$categories = array();
$sqlTogetCategories = 'select * from category where id > 0 order by id';
$categories = $db->query($sqlTogetCategories);
//print_r($categories);
?>
<style type="text/css">
.select_cat_list{
...
<h1>Move Files</h1>
<form action="syncronize_db.php" name="syncronize_db">
	<div>Select Category</div>
	<div>
    	<select name="parent_cat_id">
        <option value="0"> / </option>
        <?php foreach($categories as $catKey=>$catVal): ?>
        <option value="<?php echo $catVal['id']; ?>"
        <?php if($catVal['id'] == $_REQUEST['parent_cat_id']) echo "selected=selected"; ?>
         > 
			<?php echo $catVal['clink']; ?> 
        </option>
        <?php endforeach; ?>
        </select>
    </div>
...
//print_r($folder_scan_file);exit;
foreach ($folder_scan_file as $key => $val) {
    $parentid = $_REQUEST['parent_cat_id'];
...
$row = array();
$sqlTogetCategoriy = 'select * from category where id ='. $_REQUEST['parent_cat_id'];
$row = $db->query($sqlTogetCategoriy, database::GET_ROW);

if(count($row) > 0):
$totalitemSum = $row['totalitem'] + $total_file_insert;
$sqlUpdateCategoriyTotalitems = "update category set totalitem = $totalitemSum where id  = ".$_REQUEST['parent_cat_id'];
$db->query($sqlUpdateCategoriyTotalitems);
endif;
