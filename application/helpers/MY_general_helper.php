 ...
 * @return mixed
 */
function handlePaymentSuccess($callback_data, $payment_gateway_id) {
...
//cidb($callback_data); exit;
    ...
    foreach ($items as $itemKey => $item) {
        ...
        $sub_order_detail['customer_id'] = $customer['customer_id'];
        ...
        if ($ci->cart->getdiscounts()) {
            ...
            $update_suborder_discount_data['sub_order_total'] = ((float) ($item['subtotal'] - $totladiscount)) + (float) (($charity > 0) ? ($charity / count($items)) : 0);
...
function getShootCount($startDate, $endDate, $condition) {
	...
	WHERE DATE(os.order_date) >= DATE('{$startDate}') AND DATE(os.order_date) <= DATE('{$endDate}')" . $condition;
    ...
    foreach ($getShoot as $key => $val) {
        ...
        if (isset($getShoot[$key]['service_attributes_values_array']['Type of Shoot'])) {
            ...
            if ($val['service_id'] == RETOUCHING || $val['service_id'] == 147) {
                $RetouchingCount ++;
            }
        }
    }
...
function pe_order_date_format($date) {
    if ($date == '' || $date == '0000-00-00 00:00:00') {
        ...
        return date('d M Y h:i a', strtotime($date)) . ' Pacific';
    }
}
